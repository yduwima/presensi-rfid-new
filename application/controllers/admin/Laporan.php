...
class Laporan extends CI_Controller {
    ...
    public function pdf_absensi_guru()
    {
        ...
        $bulan_nama = date('F', mktime(0, 0, 0, $bulan, 1));
        $title = 'LAPORAN ABSENSI GURU DAN STAFF - ' . strtoupper($bulan_nama) . ' ' . $tahun;
        $this->pdf_generator->add_school_header($settings, $title);
        
        // Table header
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(10, 7, 'No', 1, 0, 'C');
        $pdf->Cell(35, 7, 'NIP', 1, 0, 'C');
        $pdf->Cell(55, 7, 'Nama', 1, 0, 'C');
        $pdf->Cell(30, 7, 'Tanggal', 1, 0, 'C');
        $pdf->Cell(25, 7, 'Masuk', 1, 0, 'C');
        $pdf->Cell(25, 7, 'Pulang', 1, 1, 'C');
        ...
        foreach ($absensi as $row) {
            $pdf->Cell(10, 6, $no++, 1, 0, 'C');
            $pdf->Cell(35, 6, $row->nip, 1, 0);
            $pdf->Cell(55, 6, substr($row->nama, 0, 30), 1, 0);
            $pdf->Cell(30, 6, date('d/m/Y', strtotime($row->tanggal)), 1, 0);
            $pdf->Cell(25, 6, $row->jam_masuk ?: '-', 1, 0, 'C');
            $pdf->Cell(25, 6, $row->jam_pulang ?: '-', 1, 1, 'C');
        }
        
        $filename = 'Laporan_Absensi_Guru_' . $bulan . '_' . $tahun . '.pdf';
        $this->pdf_generator->output($filename, 'I');
    }
    
    // PDF Rekap Jurnal
    public function pdf_rekap_jurnal()
    {
        $bulan = $this->input->get('bulan') ?: date('m');
        $tahun = $this->input->get('tahun') ?: date('Y');
        ...
        // Get data
        $this->db->select('jurnal.*, guru.nama as nama_guru, mata_pelajaran.nama as nama_mapel, kelas.nama_kelas');
        $this->db->from('jurnal');
        ...
        $bulan_nama = date('F', mktime(0, 0, 0, $bulan, 1));
        $title = 'REKAP JURNAL MENGAJAR - ' . strtoupper($bulan_nama) . ' ' . $tahun;
        $this->pdf_generator->add_school_header($settings, $title);
        ...
        $pdf->Cell(50, 7, 'Guru', 1, 0, 'C');
        $pdf->Cell(40, 7, 'Mapel', 1, 0, 'C');
        $pdf->Cell(30, 7, 'Kelas', 1, 0, 'C');
        $pdf->Cell(100, 7, 'Materi', 1, 1, 'C');
        ...
        foreach ($jurnal as $row) {
            ...
            $pdf->Cell(50, 6, substr($row->nama_guru, 0, 25), 1, 0);
            $pdf->Cell(40, 6, substr($row->nama_mapel, 0, 20), 1, 0);
            $pdf->Cell(30, 6, $row->nama_kelas, 1, 0);
            $pdf->Cell(100, 6, substr($row->materi, 0, 60), 1, 1);
        }
        
        $filename = 'Rekap_Jurnal_' . $bulan . '_' . $tahun . '.pdf';
        $this->pdf_generator->output($filename, 'I');
    }
    
    // Excel Export - Absensi Siswa
    public function excel_absensi_siswa()
    {
        $bulan = $this->input->get('bulan') ?: date('m');
        $tahun = $this->input->get('tahun') ?: date('Y');
        $kelas_id = $this->input->get('kelas_id');
        ...
        $this->excel_export->export_absensi_siswa($absensi, $bulan, $tahun, $kelas_nama);
    }
    
    // Excel Export - Absensi Guru
    public function excel_absensi_guru()
    {
        $bulan = $this->input->get('bulan') ?: date('m');
        $tahun = $this->input->get('tahun') ?: date('Y');
        ...
        $this->excel_export->export_absensi_guru($absensi, $bulan, $tahun);
    }
    
    // Excel Export - Rekap Jurnal
    public function excel_rekap_jurnal()
    {
        ...
        if ($guru_id) {
            $this->db->where('jadwal_pelajaran.guru_id', $guru_id);
            $guru = $this->Guru_model->get_by_id($guru_id);
            $guru_nama = $guru ? $guru->nama : 'Semua Guru';
        } else {
            $guru_nama = 'Semua Guru';
        }
        
        $this->db->order_by('jurnal.tanggal', 'DESC');
        $jurnal = $this->db->get()->result();
        
        $this->excel_export->export_rekap_jurnal($jurnal, $bulan, $tahun, $guru_nama);
    }
